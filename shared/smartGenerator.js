 ...
 * smartGenerator.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Three-mode smart password generator.
 *
 * All randomness is from crypto.getRandomValues() — no Math.random().
 *
 * Modes
 * ─────
 ...
 * Auto-retry
 * ──────────
 * Each mode validates the candidate with computeScore(). If the score is
 * below MIN_SCORE (default 80) the generator retries up to MAX_ATTEMPTS
 * times. This is fast because generation is synchronous and cheap.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// computeScore() requires usernameResult.usernameScore to be a number.
...
/** Fisher-Yates shuffle (in-place, crypto). */
...
/**
 * Mode 1 — Maximum Security
 * Pure character-pool password with guaranteed class coverage.
 */
 ...
 * N random words joined by separator, optionally with a number suffix.
 */
function genPassphrase(opts = {}) {
  ...
  if (opts.digits !== false) {
    pw += sep + (10 + rand(90)); // 10-99
  }
  return pw;
}

/**
 * Mode 3 — Smart Memorable
 * Title-cased words + number suffix + optional symbol.
 */
function genSmartMemorable(opts = {}) {
  ...
  // Pick words, optionally filtered by category
  let pool = ALL_WORDS;
  if (opts.category && WORD_BANK[opts.category]) {
    pool = WORD_BANK[opts.category];
  }
...
/**
 * Generate a password using the specified mode, retrying until score >= MIN_SCORE.
 *
 * @param {'maxSecurity'|'passphrase'|'smartMemorable'} mode
 * @param {object} opts  Mode-specific options (see each genXxx function)
 * @returns {{ password: string, score: number, category: string, entropy: number, charsetSize: number, length: number }}
 */
...
/**
 * Quick score helper — returns the full score result for any password string.
 * Used for live re-analysis when the user edits the generated password.
 *
 * @param {string} pw
 * @returns {{ score, category, color, cssClass, breakdown, entropy, charsetSize, length }}
 */
