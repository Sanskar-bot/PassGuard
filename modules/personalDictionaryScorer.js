...
export function computeRiskLevel(found, rank) {
  if (!found)          return { label: 'Safe',     cssClass: 'risk-safe',     emoji: '🟢' };
  if (rank <= 100)     return { label: 'Critical',  cssClass: 'risk-critical', emoji: '🔴' };
  if (rank <= 1000)    return { label: 'High',      cssClass: 'risk-high',     emoji: '🟠' };
  if (rank <= 5000)    return { label: 'Medium',    cssClass: 'risk-medium',   emoji: '🟡' };
  return                      { label: 'Low',       cssClass: 'risk-low',      emoji: '🔵' };
}
